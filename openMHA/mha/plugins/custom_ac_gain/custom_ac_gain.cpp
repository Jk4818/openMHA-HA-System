...
class custom_ac_gain_t : public MHAPlugin::plugin_t<int>
{
...
public:
    ...
    custom_ac_gain_t(MHA_AC::algo_comm_t &iac, const std::string &configured_name);
...
custom_ac_gain_t::custom_ac_gain_t(MHA_AC::algo_comm_t &iac,
                             const std::string &configured_name)
    : MHAPlugin::plugin_t<int>("This plugin multiplies the sound signal"
                               " in both audio channels by a factor",
                               iac),
...
void custom_ac_gain_t::prepare(mhaconfig_t &signal_info)
{
...
void custom_ac_gain_t::release(void)
{
    scale_ch.set_range("[0,[");
}

mha_wave_t *custom_ac_gain_t::process(mha_wave_t *signal)
{
    unsigned int frame;
    for (frame = 0; frame < signal->num_frames; frame++)
    {
        value(signal, frame, 0) *= factor.data;
        value(signal, frame, 1) *= factor.data;
    }
    return signal;
}

MHAPLUGIN_CALLBACKS(custom_ac_gain, custom_ac_gain_t, wave, wave)
MHAPLUGIN_DOCUMENTATION(custom_ac_gain,
                        "example level-modification audio-channels with ac",
                        "")
